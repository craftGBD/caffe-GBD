...
template <typename Dtype>
class FrcnnPrefetchingDataLayer :
    public BaseDataLayer<Dtype>, public InternalThread {
 public:
  explicit FrcnnPrefetchingDataLayer(const LayerParameter& param)
      : BaseDataLayer<Dtype>(param) {}
  // LayerSetUp: implements common data layer setup functionality, and calls
  // DataLayerSetUp to do special data layer setup for individual layer types.
  // This method may not be overridden.
 ...
 protected:
  Blob<Dtype> prefetch_data_;
  Blob<Dtype> prefetch_label_;
  Blob<Dtype> prefetch_rois_;
  Blob<Dtype> prefetch_bbox_targets_;
  Blob<Dtype> prefetch_bbox_weights_;
  Blob<Dtype> prefetch_gt_boxes_map_;
  Blob<Dtype> transformed_data_;
  Blob<Dtype> transformed_label_;
  Blob<Dtype> transformed_rois_;
  Blob<Dtype> transformed_bbox_targets_;
  Blob<Dtype> transformed_bbox_weights_;
};

/**
 * @brief Provides data to the Net from rois of images files, specified
 *        by a frcnn train data file. For training fast-rcnn on multi-gpu.
 *
 * TODO(dox): thorough documentation for Forward and proto params.
 */
template <typename Dtype>
class FrcnnTrainDataLayer : public FrcnnPrefetchingDataLayer<Dtype> {
 public:
  explicit FrcnnTrainDataLayer(const LayerParameter& param)
      : FrcnnPrefetchingDataLayer<Dtype>(param) {}
  virtual ~FrcnnTrainDataLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "FrcnnTrainData"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int ExactNumTopBlobs() const { return 5; }  // data, rois, labels, bbox_targets, bbox_weights

 protected:
  virtual void ShuffleData();
  virtual void ShuffleData2();
  virtual vector<int> Randperm(int, int);
  virtual void InternalThreadEntry();

  shared_ptr<Caffe::RNG> prefetch_rng_;
  vector<std::pair<std::string, vector<int> > > image_database_; // im_path, <channels, height, width, flipped>
  vector<std::pair<std::string, vector<int> > > image_database_2_;
  enum WindowField { LABEL, OVERLAP, X1, Y1, X2, Y2, DX, DY, DW, DH, NUM };
  vector<vector<vector<float> > > roidb_;
  vector<vector<vector<float> > > roidb_2_;
  int cur_;
  int cur_2_;
  int skip_;
 ...
 */
template <typename Dtype>
class FrcnnDataLayer : public FrcnnPrefetchingDataLayer<Dtype> {
 public:
  explicit FrcnnDataLayer(const LayerParameter& param)
      : FrcnnPrefetchingDataLayer<Dtype>(param) {}
  virtual ~FrcnnDataLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "FrcnnData"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  // data, rois, labels, bbox_targets, bbox_weights
 ...
 protected:
  ...
  shared_ptr<Caffe::RNG> prefetch_rng_;
  // im_path, <channels, height, width, flipped>
