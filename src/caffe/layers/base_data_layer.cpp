...
// FrcnnPrefetchingDataLayer
template <typename Dtype>
void FrcnnPrefetchingDataLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  BaseDataLayer<Dtype>::LayerSetUp(bottom, top);
  // Now, start the prefetch thread. Before calling prefetch, we make two
  // cpu_data calls so that the prefetch thread does not accidentally make
  ...
  if (this->output_labels_) {
    this->prefetch_label_.mutable_cpu_data();
    this->prefetch_rois_.mutable_cpu_data();
    this->prefetch_bbox_targets_.mutable_cpu_data();
    this->prefetch_bbox_weights_.mutable_cpu_data();
  }
  DLOG(INFO) << "Initializing prefetch";
  this->CreatePrefetchThread();
  DLOG(INFO) << "Prefetch initialized.";
...
template <typename Dtype>
void FrcnnPrefetchingDataLayer<Dtype>::CreatePrefetchThread() {
  if(this->layer_param_.transform_type() == LayerParameter_TransformType_NATIVE)
     this->data_transformer_->InitRand();
  else
...
template <typename Dtype>
void FrcnnPrefetchingDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(WaitForInternalThreadToExit()) << "Thread joining failed";
}

template <typename Dtype>
void FrcnnPrefetchingDataLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  // First, join the thread
  JoinPrefetchThread();
  DLOG(INFO) << "Thread joined";
...
STUB_GPU_FORWARD(FrcnnPrefetchingDataLayer, Forward);
#endif

INSTANTIATE_CLASS(BaseDataLayer);
INSTANTIATE_CLASS(BasePrefetchingDataLayer);
INSTANTIATE_CLASS(FrcnnPrefetchingDataLayer);
